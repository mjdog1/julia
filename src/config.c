...
static void config_init(config_t* cfg) {
    memset(cfg, 0, sizeof(config_t));   //初始化
}
//配置文件的载入
int config_load(config_t* cfg) {
    config_init(cfg);			//初始化填充
    const char* cfg_file = INSTALL_DIR "config.json";  //配置文件目录 

    if (cfg->text) {
        free(cfg->text);
    }
    cfg->text = juson_load(cfg_file); //返回一个字符串
    if (cfg->text == NULL) {
    ...
    juson_doc_t json;
    juson_value_t* root = juson_parse(&json, cfg->text); //由字符串得到json
