...
static string_t mime_tb [][2] = {  //对文档的扩展名,映射一个 MIME类型
    {STRING_INIT("htm"),    STRING_INIT("text/html")},   //对应的映射?
...
void mime_map_init(void) {	//MIME(多用途互联网邮件扩展)
    int n = sizeof(mime_tb) / sizeof(mime_tb[0]); //类型个数
    for (int i = 0; i < n; ++i) {
        map_val_t val;
        val.mime = mime_tb[i][1];	//得到每一个mime
        map_put(&mime_map, &mime_tb[i][0], &val);  //进行映射
    }
}
