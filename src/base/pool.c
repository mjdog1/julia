...
int pool_init(pool_t* pool, int width, int chunk_size, int nchunks) {
    pool->width = max(width, sizeof(chunk_slot_t)); //得到一个宽度大小
    pool->chunk_size = chunk_size;		    //chunk_size表示块大小,即这个数据结构的大小
    pool->nallocated = 0;
    pool->cur = NULL;
    
    int err = vector_init(&pool->chunks, sizeof(chunk_t), nchunks);  //chunks指向第一个指针,第二个chunk_t表示宽度,nchunks表示个数
    if (nchunks == 0)
        return err;
