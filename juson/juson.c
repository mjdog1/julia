...
static int next(juson_doc_t* doc) { //解析空白行,注释等内容
    const char* p = doc->p; //p指向内容
    while (1) {
        while (*p == ' ' || *p == '\t' 
                || *p == '\r' || *p == '\n') { //空格,横向,换行和回车略过
            if (*p == '\n')
...
static int try(juson_doc_t* doc, char x) {
    // Check empty array
    const char* p = doc->p;  //字符串
    int line = doc->line;    
    char c = next(doc);	    
    if (c == x)	    //返回{
        return 1;    
    doc->p = p;	    //进一步去除了空格注释等的影响
    doc->line = line;  //当前的行数,即恢复
    return 0;
}

static juson_value_t* juson_new(juson_doc_t* doc, juson_type_t t) {
    juson_value_t* val = juson_alloc(doc);
    memset(val, 0, sizeof(juson_value_t)); //清空值
    val->t = t;
    return val;
}
...
char* juson_load(const char* file_name) {
    ...
    len = ftell(file); //得到文件的长度
    fseek(file, 0, SEEK_SET);//设置为文件的开头
    char* p = malloc(len + 1);
    if (p == NULL) {
        fclose(file); 
        return NULL;
    }
    if (fread(p, len, 1, file) != 1) { //得到文件内容,即得到的items
        free(p);	
        fclose(file);
        return NULL;
    }
...
static void juson_chunk_init(juson_chunk_t* chunk) {
    ...
    // Init slots
    juson_slot_t* slot = chunk->slots; //得到第一个槽
    for (int i = 0; i < JUSON_CHUNK_SIZE - 1; ++i) {  //为了初始化这个结构,next初始化的时候并没有值
        slot->next = slot + 1;
        slot = slot->next;
    }
    slot->next = NULL;
...
static void juson_pool_init(juson_pool_t* pool) {
    pool->allocated_n = 0;
    juson_chunk_init(&pool->head); //初始化
    pool->cur = pool->head.slots;  //pool->head.slots,当前的头就是head中的第一个
}
//分配大小?
static juson_value_t* juson_alloc(juson_doc_t* doc) {
    juson_pool_t* pool = &doc->pool; //得到池子
    if (pool->cur == NULL) { //当前池子为空
        juson_chunk_t* chunk = malloc(sizeof(juson_chunk_t)); //分配一个块
        juson_chunk_init(chunk);		    // 初始化chunk
        pool->cur = chunk->slots;		    // 当前的chunk的值,就是第一个pool的值
        
        // Insert new chunk after the head
        chunk->next = pool->head.next;		    //再之后插入新的chunk
        pool->head.next = chunk;		    //
    }
    ++pool->allocated_n;			     //分配的数量增加1
    juson_value_t* ret = &pool->cur->val;	    //当前的值
    pool->cur = pool->cur->next;		    //向后移动一个
    return ret;
}
...
juson_value_t* juson_parse(juson_doc_t* doc, const char* json) { //解析json
    doc->val = NULL;
    doc->p = json;  //json的值
    doc->line = 1;  //行数
    juson_pool_init(&doc->pool);//初始化内容
    juson_value_t* root = juson_parse_value(doc); //得到doc的值,进行之后的操作
    if (root) {
...
static juson_value_t* juson_parse_pair(juson_doc_t* doc) {
    ...
    pair->key = juson_parse_string(doc); //解析字符串
    if (pair->key == NULL)
        JUSON_FAIL(pair);
    JUSON_EXPECT(pair, next(doc) == ':', "expect ':'"); //希望解析:
    pair->val = juson_parse_value(doc);			//解析值
    if (pair->val == NULL)
        JUSON_FAIL(pair);
    return pair;
}
...
static juson_value_t* juson_parse_value(juson_doc_t* doc) {
    switch (next(doc)) {
	case '{': return juson_parse_object(doc); //对象:键值对
    case '[': return juson_parse_array(doc);	  //解析数组
    case '\"': return juson_parse_string(doc);	    //解析字符串
